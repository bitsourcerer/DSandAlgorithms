...
template <class C>
typename C::value_type choose(const typename C::value_type &v1, const typename C::value_type &v2)
{
	static std::default_random_engine re;
	static auto prob = std::bind(std::uniform_int_distribution<bool>(0,1), re)

	return prob() ? v1 : v2;
}

template <class C> std::pair<typename C::value_type, typename C::value_type>
choose(const typename C::value_type &v1, const typename C::value_type &v2)
{
	static std::default_random_engine re;
	static auto prob = std::bind(std::uniform_int_distribution<bool>(0,1), re)

	return prob() ? {v2, v1} : {v1, v2}; // if true switch their order thats it.
}
...
{
	unsigned i = lo, j = mid + 1;
	for(unsigned k = 0; k < hi; k+=2)
	{
		if(i > mid) con[k] = aux[j++];
    	else if(j > hi) con[k] = aux[i++];
    	else {
    		auto order = choose(aux[i++], aux[j++]);
    		con[k] = order.first;
    		con[k+1] = order.second;
    	}
	}
}
